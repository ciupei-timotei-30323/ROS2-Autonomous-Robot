...
- **Autonomous Navigation**: Send `GO [destination]` (e.g., `GO Desk 1`). Coordinates are loaded dynamically from `src/delivery_robot_core/config/Locations.json` (meaning you can update coordinates on the fly without rebooting the node). If the robot gets stuck while navigating, it will alert the TCP client and attempt to re-plan every 5 seconds. If it fails to find a path after 4 attempts, it will enter the `IDLE` state to conserve battery and send an alert.
